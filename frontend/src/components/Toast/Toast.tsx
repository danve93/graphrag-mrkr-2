...
export default function Toast({ id, type, message, description, onDismiss }: ToastProps) {
  const styles = {
    success: {
      ...
      icon: <CheckCircleIcon className="h-5 w-5 text-green-600 dark:text-green-400 flex-shrink-0 mt-0.5" />,
    ...
    error: {
      ...
      icon: <XCircleIcon className="h-5 w-5 text-red-600 dark:text-red-400 flex-shrink-0 mt-0.5" />,
    ...
    warning: {
      ...
      icon: <ExclamationTriangleIcon className="h-5 w-5 text-yellow-600 dark:text-yellow-400 flex-shrink-0 mt-0.5" />,
