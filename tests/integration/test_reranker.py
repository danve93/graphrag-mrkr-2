...
def test_hybrid_retrieval_uses_reranker(monkeypatch):
    ...
    cfg_settings.flashrank_enabled = True
    cfg_settings.flashrank_max_candidates = 10
    ...
    async def fake_chunk_retrieval(query, search_query=None, top_k=5, allowed_document_ids=None, query_embedding=None):
        return chunks

    async def fake_entity_retrieval(query, search_query=None, top_k=5, allowed_document_ids=None):
        return []
    ...
    # Cleanup toggle
    cfg_settings.flashrank_enabled = False
